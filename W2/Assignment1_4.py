...
plt.figure(3)
plt.clf()
plt.plot(Temperature, nu, '-', label="Kinematic Viscosity vs Temperature")
plt.legend()
plt.grid(True)
...
# making a figure inlclduing figure 1 and 2 in the same plot (two different y axis)
fig, ax1 = plt.subplots()
color = 'tab:red'
ax1.set_xlabel('time [s]')
ax1.set_ylabel('Power [W]', color=color)
ax1.plot(time[1:], Power[1:], color=color)
ax1.tick_params(axis='y', labelcolor=color)
ax2 = ax1.twinx()
color = 'tab:blue'
ax2.set_ylabel('Temperature [C]', color=color)
ax2.plot(time[1:], Temperature[1:]-273.15, color=color)
...
# plt.figure(4)
# plt.clf()
# plt.plot(time, nu, '-', label="time vs viscosity")
# plt.legend()
# plt.grid(True)
# plt.xlabel('time [s]')
# plt.ylabel('viscocity [cSt]')
# plt.show()